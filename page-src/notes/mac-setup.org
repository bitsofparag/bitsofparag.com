...
** Install browsers
   ...
   2. /(Optional but not recommended)/ Install [[https://www.google.com/intl/de/chrome/dev/][Chrome Dev]] with =brew cask install google-chrome-dev=.
...
** SSH Keys
   ...
   If you need to add the public key to a host, run:
   #+begin_src sh
     ssh-copy-id -f -i path/to/pubkey username@remote_host
   #+end_src

   Also, add ssh pub keys to Gitlab and/or Github.

** Set up Git & other useful tooling
   1. First, install [[https://www.gnu.org/software/stow/][GNU Stow]] with =brew install stow=.
   2. =mkdir -p ~/Workspace ~/Workspace/work ~/Workspace/play
      ~/Workspace/vendor=
   3. =cd ~/Workspace/play=
   4. =git clone git@github.com:bitsofparag/dotfiles.git=
   ...
   10. Install updated Gem from brew with =brew install gem=. Make sure the
       correct ruby version is picked up in $PATH. Normally brew installs =gem=
...
** Shell setup
   1. Open Terminal and install brew with this command:
      #+begin_example
        /usr/bin/ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)"
      #+end_example
   2. Open Terminal and install =brew cask install iterm2=.
   3. Open =iterm2= and set to zsh by: =chsh -s /bin/zsh=.

   4. Check =$PATH=. It should be something like
      #+begin_src sh
        /usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin
      #+end_src

   5. Remove =/etc/paths= file.
   6. Install [[https://fonts.google.com/specimen/IBM+Plex+Mono][IBM Plex Mono]] font and set as the default font for iterm.
   7. Install "Oh My Zsh" - =sh -c "$(curl -fsSL https://raw.github.com/robbyrussell/oh-my-zsh/master/tools/install.sh)"=.
   8. Install some cool fonts:
      #+begin_src sh
        cd Workspace/vendor
        git clone https://github.com/powerline/fonts.git
        cd fonts && ./install.sh
      #+end_src
      Check the Font Book app to verify that the fonts were installed.
      1. Install iTerm color schemes - download the zip file from [[https://github.com/mbadolato/iTerm2-Color-Schemes][here]].
         Open the schemes folder and import the ones you like from iTerm's preferences - Profiles > Colors > Color Presets.
      2. Download a zip package (a very large one) of nerd fonts from [[https://github.com/ryanoasis/nerd-fonts#font-installation][this repo]].
      3. Unzip this package and copy "install.sh" and "patched-fonts/FiraCode" to "Workspace/vendor".
      4. Install this FiraCode nerd font - =./install.sh FiraCode=.
      5. Install [[https://github.com/Powerlevel9k/powerlevel9k/wiki/Install-Instructions#step-1-install-powerlevel9k][Powerlevel9k themes]]:
         #+begin_src sh
	   git clone https://github.com/bhilburn/powerlevel9k.git ~/.oh-my-zsh/custom/themes/powerlevel9k
         #+end_src
      6. Open =.zshrc= and set =ZSH_THEME="powerlevel9k/powerlevel9k"=.
      7. Link the =.zshenv.powerlevel= file from your dotfiles and source it in the =.zshrc= file. Set the preferred font to "Fira Code Nerd".
      8. Close iTerm and open again. You should see the Powerlevel theme in effect.
      9. Install syntax highlighting plugin:
	       #+begin_src sh
           git clone https://github.com/zsh-users/zsh-syntax-highlighting.git \
               ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting
         #+end_src
   9. Activate the plugin in =~/.zshrc= by adding =zsh-syntax-highlighting= to the Plugins section.
   10. Similarly, install and activate =zsh-autosuggestions=:
        #+begin_src sh
          git clone https://github.com/zsh-users/zsh-autosuggestions \
              $ZSH_CUSTOM/plugins/zsh-autosuggestions
        #+end_src
   11. Install [[https://github.com/junegunn/fzf#installation][fzf]].
   12. Install fzf keybindings - =/usr/local/opt/fzf/install=.
   13. Symlink zsh config files with functions
        #+begin_src sh
          ...
          EOF
        #+end_src
   14. Install [[https://github.com/sharkdp/bat][bat]] to replace cat: =brew install bat=.
   15. Install [[https://github.com/abishekvashok/cmatrix/][CMatrix]] for fancy =clear= command (which should be aliased
        already in the dotfiles; see step 21.)
   16. Install =tree= with =brew install tree=.
   17. Install =fswatch= with =brew install fswatch=.
...
** Node and NPM setup
   ...
   1. Uninstall node and [[https://github.com/tj/n][n]]:
   2. Install [[https://docs.volta.sh/guide/getting-started][Volta]] via brew: =curl https://get.volta.sh | bash=. The installation will
      ...
      1. Install the lts and the latest versions of node:
         #+begin_src sh
         volta install node@lts  # installs lts
         volta pin node@lts
         volta list
         #+end_src
   3. Install [[https://deno.land/][Deno]] with =brew install deno=.
   4. Install [[https://pnpm.io/installation][pnpm]] with =curl -f https://get.pnpm.io/v6.16.js | node - add --global pnpm=.
   5. Install global packages:
      #+begin_src sh
        pnpm add -g @storybook/cli \
            ...
            semver
      #+end_src
   6. Check the packages installed globally:
      #+begin_src sh
      pnpm ls -depth=0 -g
      #+end_src
   7. [DEPRECATED] If you have other node versions, copy the same packages over with:
      #+begin_src sh
...
** Java setup
   1. Install java on Catalina with =brew tap AdoptOpenJDK/openjdk && brew cask install adoptopenjdk8=.
...
** Emacs setup
   ...
   10. Install BasicTeX as the latex package with =brew cask install basictex=.
...
** Postgres setup
   ...
   3. Install [[https://github.com/Paxa/postbird][Postbird app]] as the GUI utility for Postgres with =brew cask install postbird=. [UNINSTALLED]
...
** Devops setup
   ...
   3. Install aws session-manager-plugin with =brew cask install
   ...
   15. Install Virtualbox with =brew cask install virtualbox=.
   16. Install [[https://www.vagrantup.com/][Vagrant]] with =brew cask install vagrant=.
   17. [REMOVED] Install [[https://oldj.github.io/SwitchHosts/][Switchhosts]] to manage local hosts with =brew cask install switchhosts=.
   18. Install Gas Mask with =brew install --cask gas-mask=.
   19. Install [[https://www.postman.com/][Postman app]] with =brew cask install Postman=.
...
** Dropbox setup
   1. Install Dropbox: =brew cask install dropbox=.

** Password manager setup
   1. Install password managers: =brew cask install keepassxc lastpass=.
...
** Music player setup
   1. Install Spotify app - =brew cask install spotify=
...
** Graphics software setup
   - Install [[https://www.blender.org/][blender]] with =brew cask install blender=.
** Security setup
   1. Install [[https://github.com/cryptomator/cryptomator][Cryptomator]] for file encryption with =brew cask install cryptomator=.
   2. Install [[https://objective-see.com/products/lulu.html][Lulu]] as the firewall to block unknown outgoing connections, with
      =brew cask install lulu=.
   3. Install [[https://objective-see.com/products/netiquette.html][Netiquette]] as a network tracker GUI utility: =brew cask install netiquette=.
   4. Install [[https://objective-see.com/products/oversight.html][Oversight]] as webcam and mic monitoring utility: =brew cask install oversight=.
   5. Install [[https://www.malwarebytes.com/mac/][Malwarebytes]] for malware detection with =brew cask install malwarebytes=.

** Wordpress and CMS setup
   1. Install [[https://www.mamp.info/de/mac/][Mamp]] with =brew cask install mamp=.
...
** Other useful utility packages:
   ...
   8. Install [[https://www.spectacleapp.com/][Spectacle app]] as the window manager with =brew cask install spectacle=.
   9. Install [[https://github.com/JamieMason/ImageOptim-CLI#examples][imageoptim-cli]] with =brew install imageoptim-cli=.
   10. [REMOVED] Install [[https://github.com/iina/iina][iina]] as the movie player with =brew cask install iina=.
   11. (Optional) Install [[https://www.zoom.us/][Zoom.us]] with =brew cask install zoomus=.
   12. Install [[https://www.android.com/filetransfer/][Android File Transfer]] via the DMG file.
   13. Install [[https://formulae.brew.sh/cask/inkscape][inkscape]] with =brew cask install inkscape=.
